...
cBugReport getBR(string str,int type)
{
	...
	sevCount[type][sevScore[tempBR.sever]]++; //count the number of five sever labels
...
{
	/*******************************************************************
	/	wf[2] the weights of two fields
	/	bf    controls parameter range(0,1) b_desc = 1.0 b_summ = 0.5
...
{
	double L = 0.031; //controls parameter, while L==0£¬BM25Fext become BM25F
	double wf1[2] = {3.014,0.764}; 
	double TFQ = 0;

	int t1 = testBR[docID].summ_times_unigram[str]; // the times that str appears in the query.summ
	int t2 = testBR[docID].desc_times_unigram[str]; //the times that str appears in the query.desc
	TFQ += wf1[0] * t1;
	TFQ += wf1[1] * t2;
...
static double getWQ_bigram(pair<string,string> str,int docID)
{
	...
	double wf2[2] = {2.971,1.003}; 
	double TFQ = 0;
	int t1 = testBR[docID].summ_times_bigram[str]; // the times that str appears in the query.summ
	int t2 = testBR[docID].desc_times_bigram[str]; // the times that str appears in the query.desc
	TFQ += wf2[0] * t1;
	TFQ += wf2[1] * t2;
	return (L+1)*TFQ / (L+TFQ);
}
...
vector<Result> getBM25FextAndREP(cBugReport tempBR,int i) 
{
	...
	for(int j = 0;j < trainBR.size();j++)
	{
		...
		for(ite = comWord_unigram[trainBR[j].bugID].begin(); ite != comWord_unigram[trainBR[j].bugID].end(); ite++) //get the Comword of testBR[i] and trainBR[j] 
		{
